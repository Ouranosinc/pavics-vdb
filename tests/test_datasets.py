...
class TestDataset:
    ...
    def test_ESPO_R(self, compare_raw=False):
        ...
        datasets = sorted(
            list(path.Path('../tmp/simulations/bias_adjusted/cmip5/ouranos/ESPO-R/ESPO-R5v1.0.0').rglob('*.ncml')))
        ...
        if path.Path(local_path).is_dir() or path.Path(local_path).is_file() or key1 == '@regExp':
            if path.Path(local_path).is_file():
                ...
                if '@regExp' in l.keys():
                    ...
                    str1 = f"{regexp.replace(str1, '')}{str1}".replace('**', '*').replace('\\',
                                                                                          '')  # regexp can occasionally already have suffix replace double
                ...
                if 'climex' in l1:
                    ...
                    if 'cccs_portal' in l1[1]:
                        if 'realization' in dsNcML.dims:
                            ...
                            dstest = dsNcML.sel(
                                realization=[f"{str(r.values).split(':r')[0]}_" in path.Path(l[1]).name for r in
                        ...
                        if 'realization' in dsNcML.dims:
                            rr = [r for r in dsNcML.realization if
                                  any([str(r.astype(str).values).split(':')[0] == i for i in l[1].split('/')]) and
                                  any([str(r.astype(str).values).split(':')[1] == i for i in l[1].split('/')])
                                  ]
                            if len(rr) > 1:
                                raise ValueError(f"expected one realization found {rr}")
                            dstest = dsNcML.sel(
                                realization=rr[0]).squeeze()
                            compare_values(dstest, ds, compare_vals, sample_time=sample_time)
                        else:
