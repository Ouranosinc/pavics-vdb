...
def main():
    ...
    dataset_configs = p.Path(f"{home}/github/github_pavics-vdb/dataset_json_configs").rglob('*ESPO-G6*.json')
                    ...
                    if all(var_flag):
                        ...
                        for v in config['variables']:
                            ...
                            netcdf2['aggregation'] = ncml_add_aggregation(
                                {'@dimName': 'time', '@type': 'joinExisting', '@recheckEvery': '1 day'})
                            netcdf2['aggregation']['scan'] = []
                            scan = {'@location': scen.as_posix().replace(pavics_root, 'pavics-data'), '@subdirs': 'true',
                                    '@suffix': f'{v}_*.nc'}
                            netcdf2['aggregation']['scan'].append(ncml_add_scan(scan))
