...
def ncml_create_datasets(ncml_template=None, config=None):
    if config['ncml_type'] == 'cmip5-multirun-single-model':
        ...
        location = config['location'].replace('pavics-data', pavics_root)
        freq = config['frequency']
        for gcm in [x for x in p.Path(location).iterdir() if x.is_dir()]:
            hist_path = [x for x in gcm.iterdir() if x.is_dir() if x.name == 'historical']
            if len(hist_path) != 1:
                print(f'expected one path for historical files. Found {len(hist_path)} ... skipping {gcm}')
                continue
            hist_path = hist_path[0]
            for scen in [x for x in gcm.iterdir() if x.is_dir()]:
                scen
                for run in [x for x in scen.iterdir() if x.is_dir()]:
                    #hist_run = hist_path.joinpath(run.name)

                    glob_str = "{v}_*_{freq}_*.nc"
                    vars = list(set([l.name.split('_')[0] for l in list(run.rglob(f"*_{freq}_*.nc"))]))
                    var_flag = [len(list(run.rglob(glob_str.format(v=v, freq=freq)))) > 0 for v in vars]
                    #var_flag.extend([len(list(hist_run.rglob(glob_str.format(v=v, freq=freq)))) > 0 for v in vars])
                    agg_dict = {"@type": "Union"}
                    agg = ncml_add_aggregation(agg_dict)
                    agg['netcdf'] = []
                    outname = None
                    if all(var_flag):
                        ...
                        for v in vars:
                            allfiles = sorted(list(run.rglob(glob_str.format(v=v, freq=freq))))
                            if outname is None:

                                outname = f"{freq}_{'_'.join(list(run.rglob(glob_str.format(v=v, freq=freq)))[0].stem.split('_')[1:-2])}"
                                #allfiles = sorted(list(hist_run.rglob(glob_str.format(v=v, freq=freq))))

                                #allfiles = sorted(allfiles)
                                years = f"{allfiles[0].stem.split('_')[-1].split('-')[0][0:8]}-{allfiles[-1].stem.split('_')[-1].split('-')[-1][0:8]}"
                                outname = '_'.join([outname, years])


                            #fut_files = sorted(list(run.rglob(glob_str.format(v=v, freq=freq))))
                            netcdffut = ncml_netcdf_container()
                            netcdffut['aggregation'] = ncml_add_aggregation(
                                {'@dimName': 'time', '@type': 'joinExisting', '@recheckEvery': '1 day'})
                            scan = {'@location': allfiles[0].parent.as_posix().replace(pavics_root, 'pavics-data'),
                                    '@subdirs': 'false',
                                    '@suffix': '*.nc'}
                            netcdffut['aggregation']['scan'] = []
                            netcdffut['aggregation']['scan'].append(ncml_add_scan(scan))
                            # netcdffut['aggregation']['remove'] = []
                            # netcdffut['aggregation']['remove'].append({"@name": "time_bnds", "@type": "variable"})



                            agg['netcdf'].append(netcdffut)
                            agg['remove'] = []
                            agg['remove'].append({"@name": "time_bnds", "@type": "variable"})
                            #agg['netcdf']['remove'].append({"@name": "time_bnds", "@type": "variable"})
