...
def main():
    ...
    dataset_configs = rootdir.joinpath("dataset_json_configs").rglob('*ESPO*post2100*_config*.json')
...
def ncml_create_datasets(ncml_template=None, config=None):
    if config['ncml_type'] == 'cmip5-multirun-single-model':
                ...
                for run in [ l for l in exp.glob('*') if l.is_dir()]:
                    for mod in [ l for l in run.glob('*') if l.is_dir()]:
                        for ver in [l for l in mod.glob('*') if l.is_dir()]:
                            ...
                            agg = ncml_add_aggregation(agg_dict)
                            agg['netcdf'] = []
                            if "include_invariant" in config.keys():
                                if config["include_invariant"] is True:
                                    fx_list = sorted([fx.name for fx in ver.glob('fx') if fx.is_dir()])
                                else:   
                                    fx_list = config["include_invariant"]
                                
                                invariant_loc = ver.joinpath('fx')
                                for ff in fx_list:
                                    if invariant_loc.joinpath(ff).exists():
                                        for ii, nc in enumerate(sorted(list(invariant_loc.joinpath(ff).rglob('*.nc')))):
                                        ...
                                        raise ValueError(f"Invariant variable folder {invariant_loc.joinpath(ff)} does not exist.")
        ...
        agg = ncml_add_aggregation(agg_dict)
        agg['netcdf'] = []
        ncfiles = sorted(list(infolder.rglob(f'*{freq}*.nc')))
        outname_files =[n for n in ncfiles if n.name.startswith("tas")]
        if len(outname_files) == 0:
            outname_files = [n for n in ncfiles if n.name.startswith("pr")]
        assert len(outname_files) > 0, f"No files found in {infolder} for frequency {freq}"
        #time_per = f"{outname_files[0].stem.split('_')[-1].split('-')[0]}-present"
        outname = f"{'_'.join(outname_files[0].stem.split('_')[1:-1])}"
        ...
        for inst in [x for x in p.Path(location).iterdir() if x.is_dir()]:
            for rcm in [x for x in inst.iterdir() if x.is_dir()]:
                for gcm in [x for x in rcm.iterdir() if x.is_dir()]:
                    for rcp in [x for x in gcm.iterdir() if x.is_dir() and 'rcp' in x.name]:
                        for member in [x for x in rcp.iterdir() if x.is_dir()]:
                            for freq in config['frequency']:
                                var_flag = [len(list(member.joinpath(freq).rglob(f"*{v}_*.nc"))) > 0 for v in config['variables']]
                                agg_dict = {"@type": "Union"}
                                agg = ncml_add_aggregation(agg_dict)
                                agg['netcdf'] = []
                                outname = None
                                if all(var_flag):
                                    ...
                                    # ensure all variables are present
                                    for v in config['variables']:
                                        allfiles = sorted(list(member.joinpath(freq).rglob(f"*{v}_*{freq}*.nc")))
                                        if outname is None:
                                            outname = '_'.join(list(member.joinpath(freq).rglob(f"*{v}_*.nc"))[0].stem.split('_')[1:-1])
